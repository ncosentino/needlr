# NDLRGEN048: Constructor guard incompatible with member type

## Cause

A built-in `[ConstructorGuard]` kind is applied to a participating field or record
property whose type it cannot validate:

- `NotNullOrEmpty` or `NotNullOrWhiteSpace` applied to a member that is not `string`.
- `NotNull` applied to a member where a runtime `null` value is never possible: a non-nullable value type (other than `Nullable<T>`).

## Rule Description

`NotNullOrEmpty` and `NotNullOrWhiteSpace` call `System.ArgumentException.ThrowIfNullOrEmpty` / `ThrowIfNullOrWhiteSpace`, both of which only accept a `string`. Applying either to a non-`string` member would not compile as generated source, so it is rejected earlier, at the attribute usage site, with a clear message.
...
Choose a guard kind compatible with the member's type, or change the member's type:
