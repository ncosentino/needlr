...
function renderBenchmarkSections(data, descriptions, container) {
    ...
    for (const className of Object.keys(grouped).sort()) {
        ...
        const baselineMethodName = desc?.baseline;
        ...
        // Data table (pass baseline method name)
        html += renderBenchmarkTable(benchmarks, baselineMethodName);
        
        // Chart containers - find baseline for memory check
        const baseline = baselineMethodName 
            ? benchmarks.find(b => b.Method === baselineMethodName)
            : benchmarks[0];
    ...
    for (const className of Object.keys(grouped).sort()) {
        const desc = descriptions[className];
        const baselineMethodName = desc?.baseline;
        renderChartsForBenchmark(className, grouped[className], chartIndex, baselineMethodName);
        chartIndex++;
    }
}

function renderBenchmarkTable(benchmarks, baselineMethodName) {
    const baseline = baselineMethodName 
        ? benchmarks.find(b => b.Method === baselineMethodName)
        : benchmarks[0];
    ...
    benchmarks.forEach(b => {
        const isBaseline = baselineMethodName ? b.Method === baselineMethodName : b === baseline;
...
function renderChartsForBenchmark(className, benchmarks, chartIndex, baselineMethodName) {
    const baseline = baselineMethodName 
        ? benchmarks.find(b => b.Method === baselineMethodName)
        : benchmarks[0];
    if (!baseline) return;
    ...
    const methods = benchmarks.map(b => ({
        ...
        isBaseline: baselineMethodName ? b.Method === baselineMethodName : b === baseline
    })).filter(m => m.time);
    ...
    const colors = methods.map(m => m.isBaseline ? 'rgba(33, 150, 243, 0.7)' : 'rgba(46, 125, 50, 0.7)');
    const borderColors = methods.map(m => m.isBaseline ? 'rgba(33, 150, 243, 1)' : 'rgba(46, 125, 50, 1)');
...
function updateKeyTakeaway(data, element) {
    ...
    buildBenchmarks.forEach(b => {
        const className = (b.Type || 'Unknown').split('.').pop();
        if (!grouped[className]) grouped[className] = {};
        if (b.Method?.includes('ManualDI')) grouped[className].manual = b;
        else if (b.Method?.includes('SourceGen') && !b.Method?.includes('Explicit')) grouped[className].sourceGen = b;
        else if (b.Method?.includes('Reflection')) grouped[className].reflection = b;
    });
    
    for (const [benchName, benchmarks] of Object.entries(grouped)) {
        // Compare source gen to manual DI if available, otherwise to reflection
        const baseline = benchmarks.manual || benchmarks.reflection;
        if (baseline && benchmarks.sourceGen) {
            const baseTime = getMeanFromMeasurements(baseline);
            const sgTime = getMeanFromMeasurements(benchmarks.sourceGen);
            if (baseTime && sgTime && sgTime > 0) {
                const speedup = (baseTime / sgTime).toFixed(1);
    ...
    if (details.length > 0) {
        element.innerHTML = `<strong>DI container build speedups (source-gen vs manual DI):</strong> ${details.join(', ')}. Service resolution is identical once the container is built.`;
    }
}
</script>
