...
public sealed class ExperimentRunnerRetryTests
{
    ...
    [Fact]
    public async Task RunAsync_NearerRetry_WakesDispatcherBeforeLaterRetry()
    {
        var timeProvider = new FakeTimeProvider();
        var longRetrySelected = new TaskCompletionSource(
            TaskCreationOptions.RunContinuationsAsynchronously);
        var shortRetrySelected = new TaskCompletionSource(
            TaskCreationOptions.RunContinuationsAsynchronously);
        var longAttempts = 0;
        var shortAttempts = 0;
        var definition = new ExperimentDefinition<string, string>
        {
            ...
            CaseSource = new LocalExperimentCaseSource<string>(
                "local",
                [
                    new ExperimentCase<string> { Id = "long", Value = "long" },
                    new ExperimentCase<string> { Id = "short", Value = "short" },
                ]),
            Task = async (context, cancellationToken) =>
            {
                if (context.Case.Id == "long")
                {
                    var attempt = Interlocked.Increment(ref longAttempts);
                    return attempt == 1
                        ? throw new InvalidOperationException("long retry")
                        : "long";
                }

                var shortAttempt = Interlocked.Increment(ref shortAttempts);
                if (shortAttempt == 1)
                {
                    await longRetrySelected.Task.WaitAsync(cancellationToken);
                    throw new InvalidOperationException("short retry");
                }
        ...
        var retryPolicy = new ExperimentRetryPolicy(
            maxAttempts: 2,
            retryOn: ExperimentRetryableOutcome.ExecutionFailure,
            delayProvider: context =>
            {
                if (context.CaseId == "long")
                {
                    longRetrySelected.SetResult();
                    return TimeSpan.FromHours(1);
                }

                shortRetrySelected.SetResult();
        ...
        var runTask = new ExperimentRunner(timeProvider).RunAsync(
            ...
            _cancellationToken);
        await shortRetrySelected.Task.WaitAsync(_cancellationToken);

        await AdvanceUntilAsync(
            timeProvider,
            () => Volatile.Read(ref shortAttempts) == 2,
            TimeSpan.FromMinutes(1));

        Assert.Equal(1, longAttempts);
        await AdvanceUntilAsync(
            timeProvider,
            () => Volatile.Read(ref longAttempts) == 2,
            TimeSpan.FromMinutes(1));
        var result = await runTask;

        Assert.Equal(["long", "short"], result.Items.Select(item => item.Output));
    }
