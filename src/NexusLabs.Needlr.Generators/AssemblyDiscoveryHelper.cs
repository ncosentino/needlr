        ...
        foreach (var reference in compilation.References)
        {
            if (compilation.GetAssemblyOrModuleSymbol(reference) is IAssemblySymbol assemblySymbol)
            {
                ...
                foreach (var typeSymbol in TypeDiscoveryHelper.GetAllTypes(assemblySymbol.GlobalNamespace))
                {
                    ...
                    // Source/CompilationReference symbols retain locations. PE metadata
                    // symbols deliberately produce null locations for IDE graph merging.
                    var interfaceInfos = interfaceSymbols.Select(i =>
                    {
                        ...
                        var ifaceLine = ifaceLocation is null
                            ? 0
                            : ifaceLocation.GetLineSpan()
                    ...
                    var sourceLine = sourceLocation is null
                        ? 0
                        : sourceLocation.GetLineSpan()
                            .StartLinePosition.Line + 1;
