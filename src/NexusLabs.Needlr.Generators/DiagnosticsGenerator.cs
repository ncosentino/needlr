...
{
    /// <summary>
    /// Emitted in place of a wall-clock timestamp so generated source stays byte-identical
    /// across builds. The <c>NeedlrExtractDiagnostics</c> MSBuild task substitutes the real
    /// UTC time when it writes the markdown reports to disk.
    /// </summary>
    internal const string GeneratedAtPlaceholder = "__NEEDLR_GENERATED_AT__";
    ...
    internal static string GenerateDiagnosticsSource(DiscoveryResult discoveryResult, string assemblyName, string? projectDirectory, DiagnosticOptions options, IReadOnlyList<string> referencedTypeRegistryAssemblies, Dictionary<string, List<DiagnosticTypeInfo>> referencedAssemblyTypes)
    {
        ...
        var timestamp = GeneratedAtPlaceholder;
