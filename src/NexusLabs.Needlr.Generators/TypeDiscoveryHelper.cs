...
internal static class TypeDiscoveryHelper
{
    ...
    // Delegation methods for extracted helpers
    // These maintain backwards compatibility with existing code

    public static IReadOnlyList<OpenDecoratorDiscoveryHelper.OpenDecoratorInfo> GetOpenDecoratorForAttributes(INamedTypeSymbol typeSymbol)
        => OpenDecoratorDiscoveryHelper.GetOpenDecoratorForAttributes(typeSymbol);
    ...
    public static Dictionary<INamedTypeSymbol, List<INamedTypeSymbol>> FindClosedImplementations(
        ...
        => OpenDecoratorDiscoveryHelper.FindClosedImplementations(openGenericInterface, allTypes);

    public static IReadOnlyList<InterceptorDiscoveryHelper.InterceptorInfo> GetInterceptAttributes(INamedTypeSymbol typeSymbol)
        => InterceptorDiscoveryHelper.GetInterceptAttributes(typeSymbol);

    public static IReadOnlyList<InterceptorDiscoveryHelper.InterceptorInfo> GetMethodLevelInterceptAttributes(INamedTypeSymbol typeSymbol)
        => InterceptorDiscoveryHelper.GetMethodLevelInterceptAttributes(typeSymbol);

    public static bool HasInterceptAttributes(INamedTypeSymbol typeSymbol)
        => InterceptorDiscoveryHelper.HasInterceptAttributes(typeSymbol);

    public static IReadOnlyList<InterceptorDiscoveryHelper.InterceptedMethodInfo> GetInterceptedMethods(
        INamedTypeSymbol typeSymbol,
        IReadOnlyList<InterceptorDiscoveryHelper.InterceptorInfo> classLevelInterceptors,
        IReadOnlyList<InterceptorDiscoveryHelper.InterceptorInfo> methodLevelInterceptors)
        => InterceptorDiscoveryHelper.GetInterceptedMethods(typeSymbol, classLevelInterceptors, methodLevelInterceptors);

    public static bool HasGenerateFactoryAttribute(INamedTypeSymbol typeSymbol)
        => FactoryDiscoveryHelper.HasGenerateFactoryAttribute(typeSymbol);

    public static int GetFactoryGenerationMode(INamedTypeSymbol typeSymbol)
        => FactoryDiscoveryHelper.GetFactoryGenerationMode(typeSymbol);

    public static string? GetFactoryReturnInterfaceType(INamedTypeSymbol typeSymbol)
        => FactoryDiscoveryHelper.GetFactoryReturnInterfaceType(typeSymbol);

    public static IReadOnlyList<FactoryDiscoveryHelper.FactoryConstructorInfo> GetFactoryConstructors(INamedTypeSymbol typeSymbol)
        => FactoryDiscoveryHelper.GetFactoryConstructors(typeSymbol);

    public static int GetPluginOrder(INamedTypeSymbol typeSymbol)
        => PluginOrderHelper.GetPluginOrder(typeSymbol);

    public static bool HasOptionsAttribute(INamedTypeSymbol typeSymbol)
        => OptionsAttributeHelper.HasOptionsAttribute(typeSymbol);

    public static IReadOnlyList<OptionsAttributeHelper.OptionsAttributeInfo> GetOptionsAttributes(INamedTypeSymbol typeSymbol)
        => OptionsAttributeHelper.GetOptionsAttributes(typeSymbol);

    public static OptionsAttributeHelper.OptionsValidatorMethodInfo? FindValidationMethod(INamedTypeSymbol typeSymbol, string methodName = "Validate")
        => OptionsAttributeHelper.FindValidationMethod(typeSymbol, methodName);
}