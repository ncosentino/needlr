...
function hasBetterLocation(candidate, existing) {
    const candidateHasLocation = !!candidate.location?.filePath && candidate.location.line > 0;
...
function calculateStatistics(services) {
    return {
        totalServices: services.length,
        singletons: services.filter(service => service.lifetime === 'Singleton').length,
        scoped: services.filter(service => service.lifetime === 'Scoped').length,
        transient: services.filter(service => service.lifetime === 'Transient').length,
        decorators: services.reduce((sum, service) => sum + service.decorators.length, 0),
        interceptors: services.reduce((sum, service) => sum + service.interceptors.length, 0),
        factories: services.filter(service => service.metadata.hasFactory).length,
        options: services.filter(service => service.metadata.hasOptions).length,
        hostedServices: services.filter(service => service.metadata.isHostedService).length,
        plugins: services.filter(service => service.metadata.isPlugin).length
    };
}
//# sourceMappingURL=graphMerge.js.map